...
def train_model(X,y,f1,f2):
    ...
    inputs = Input(shape=(X.shape[1],X.shape[2],X.shape[3],X.shape[4])) 
    x = TimeDistributed(Conv2D(64,kernel_size = 3,activation='relu'))(inputs)
    x = BatchNormalization()(x)
    x = TimeDistributed(Conv2D(32,kernel_size = 3,activation='relu'))(inputs)
    x = BatchNormalization()(x)
    x = TimeDistributed(Flatten())(x)
    x = LSTM(64, activation='sigmoid',dropout=0.5)(x) 
    ...
    f1 = np.reshape(f1,(f1.shape[0],f1.shape[1],f1.shape[2],f1.shape[3],1))
    f2 = np.reshape(f2,(f2.shape[0],f2.shape[1],f2.shape[2],f2.shape[3],1))
